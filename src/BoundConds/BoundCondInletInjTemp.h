...
{
public:
  BoundCondInletInjTemp(int numPhysique, tinyxml2::XMLElement* element, const int& numbPhases, std::string fileName = "Unknown file");
  BoundCondInletInjTemp(const BoundCondInletInjTemp& Source, const int& lvl = 0); // Copy ctor (useful for AMR)
  ~BoundCondInletInjTemp();
  ...
  virtual int whoAmI() const { return INLETINJTEMP; };
